...
/* Maximum physical pages per segment (64 * 4096 = 256 KB). */
...
typedef struct {
    ...
    uint32_t lpid;
    int      valid;
    int      destroy;   /* IPC_RMID called; free when nattch drops to 0 */
} shm_seg_t;
...
/* Free physical pages and mark slot as free. Caller must hold shm_lock. */
static void seg_free(shm_seg_t* s) {
    for (uint32_t i = 0; i < s->num_pages; i++) {
        if (s->pages[i]) { kfree_page(s->pages[i]); s->pages[i] = 0; }
    }
    s->valid  = 0;
    s->nattch = 0;
}
...
int shm_get(int key, uint32_t size, int flags) {
    ...
    if (key != IPC_PRIVATE) {
        for (int i = 0; i < SHM_MAX_SEGMENTS; i++) {
            if (!shm_table[i].valid || shm_table[i].key != key) continue;
    ...
    if (size == 0) { irq_spinlock_release(&shm_lock); return -1; }

    int slot = -1;
    for (int i = 0; i < SHM_MAX_SEGMENTS; i++) {
        if (!shm_table[i].valid) { slot = i; break; }
    }
    if (slot < 0) { irq_spinlock_release(&shm_lock); return -1; }

    uint32_t npages = (size + PAGE_SIZE - 1) / PAGE_SIZE;
    if (npages > SHM_MAX_PAGES) { irq_spinlock_release(&shm_lock); return -1; }
    ...
    shm_table[slot].num_pages = npages;
...
/* Find a free VA in the SHM range. Caller must hold shm_lock. */
...
uint32_t shm_at(int shmid, uint32_t shmaddr, int flags) {
    ...
    if (slot < 0) { irq_spinlock_release(&shm_lock); return (uint32_t)-1; }

    shm_seg_t* seg = &shm_table[shmid - 1];

    uint32_t va;
    if (shmaddr != 0) {
        if (flags & SHM_RND) shmaddr &= ~(PAGE_SIZE - 1u);
        ...
        if (va == 0) { irq_spinlock_release(&shm_lock); return (uint32_t)-1; }
    }
...
int shm_dt(uint32_t shmaddr) {
    ...
    if (seg->destroy && seg->nattch == 0) seg_free(seg);
...
int shm_ctl(int shmid, int cmd, void* buf) {
    ...
    if (!seg_valid(shmid)) { irq_spinlock_release(&shm_lock); return -1; }

    shm_seg_t* seg = &shm_table[shmid - 1];

    if (cmd == IPC_RMID) {
        seg->destroy = 1;
        if (seg->nattch == 0) seg_free(seg);
        irq_spinlock_release(&shm_lock);
        return 0;
    }

    if (cmd == IPC_STAT) {
        if (!buf) { irq_spinlock_release(&shm_lock); return -1; }
...
void shm_detach_all(uint32_t pid, uint32_t* page_directory) {
    ...
    for (int i = 0; i < TASK_SHM_MAX; i++) {
        ...
        shm_seg_t* seg = &shm_table[id - 1];
        shm_unmap_from(page_directory, found->shm_attachments[i].shm_vaddr, seg->num_pages);
        found->shm_attachments[i].shm_id    = 0;
        found->shm_attachments[i].shm_vaddr = 0;
        if (seg->nattch > 0) seg->nattch--;
        seg->lpid = pid;
