...
uint16_t fat16_get_next_cluster(uint16_t current_cluster);

struct vfs_node* fat16_finddir(struct vfs_node* node, char* name);
int fat16_read_file(struct vfs_node* node, uint32_t offset, uint32_t size, char* buffer);
void fat16_list_dir(struct vfs_node* node);

int fat16_write_file(struct vfs_node* node, uint32_t offset, uint32_t size, char* buffer);
