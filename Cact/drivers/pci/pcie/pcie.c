...
bool pcie_init(void)
{
    ...
    if (ACPI_FAILURE(status) || !mcfg) {
        klog(LOG_WARN, "PCIe: MCFG table not found — using legacy PCI");
        return false;
    }
    ...
    if (mapped > 0) {
        pcie_ready = 1;
        klog(LOG_OK, "PCIe: ECAM enabled");
        return true;
    }

    klog(LOG_WARN, "PCIe: MCFG table invalid");
    return false;
}
