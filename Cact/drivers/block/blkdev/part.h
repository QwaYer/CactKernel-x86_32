...
// and exposes it as a /dev/<name> block node through vfsdev so that userland
// can read/write raw partition sectors.  Rescan of a disk drops the old
// partition devices first, so an installer can rewrite a partition table and
// ask the kernel to re-expose it without a reboot.

// Scan one whole-disk device. Creates partition blkdev entries and their
// vfsdev block nodes. Returns the number of partitions found (0 if none / no
// label), or a negative error code.
...
// Drop partitions of `disk` (vfsdev block nodes + blkdev entries).
