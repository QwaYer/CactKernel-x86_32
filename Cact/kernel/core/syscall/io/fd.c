...
int sys_fcntl(int fd, int cmd, int arg) {
    ...
    case 4: { // F_SETFL
        ...
        f->flags = new_flags;
        /* The socket read/write paths live in Rust (AF_INET) or in unix_sock.c
         * (AF_UNIX) and only see the node, so O_NONBLOCK has to be pushed into
         * the socket itself: without this, fcntl(F_SETFL, O_NONBLOCK) on a
         * socket fd was accepted and then ignored, and every read kept
         * blocking.  Both flavours are dispatched by their own ops. */
        if (f->node && f->node->type == VFS_SOCKET) {
            if (unix_sock_is_node(f->node))
                (void)unix_sock_set_nonblock(f->node, (int)(new_flags & O_NONBLOCK));
            else
